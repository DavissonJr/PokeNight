...
function UIMiniWindow:setupOnStart()
  ...
  if settings[char] then
    ...
    if selfSettings and selfSettings.minimized then
      self:minimize(true)
    elseif selfSettings and selfSettings.height then
    ...
    if selfSettings and selfSettings.closed then
      self:close(true)
    else
      self:open(true)
    end
...
function UIMiniWindow:setSettings(data)
  ...
  for key, value in pairs(data) do
    if char ~= nil and id ~= nil and key ~= nil and value ~= nil then
      if not settings then
        settings = {}
      end

      if settings[char] ~= nil and settings[char][id] ~= nil then
