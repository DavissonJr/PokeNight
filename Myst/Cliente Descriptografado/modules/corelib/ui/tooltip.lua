...
function onWidgetHoverChange(widget, hovered)
  if hovered then
    if widget.tooltip and not g_mouse.isPressed() then
      ...
      -- Thalles Vitor
      if string.find(currentHoveredWidget:getId(), "item") or string.find(currentHoveredWidget:getId(), "slot") then
        local item = currentHoveredWidget:getItem()
        if item then
          local newItem = g_ui.createWidget("Item", toolTipLabel)
          newItem:addAnchor(AnchorTop, "parent", AnchorTop)
          newItem:addAnchor(AnchorLeft, "parent", AnchorLeft)
          newItem:setMarginTop(7)
          newItem:setMarginLeft(5)
          newItem:setItemId(item:getPortrait())
        end
      end
    end
  else
...
function g_tooltip.init()
  ...
  addEvent(function()
    toolTipLabel = g_ui.createWidget('UIButton', rootWidget)
    toolTipLabel:setId('toolTip')
    toolTipLabel:setImageSource("/images/ui/menubox")
    toolTipLabel:setImageBorder(10)
    toolTipLabel:setTextAlign(AlignCenter)
    toolTipLabel:setFocusable(false)
...
function g_tooltip.display(text)
  ...
  toolTipLabel:resize(toolTipLabel:getWidth() + 84, toolTipLabel:getHeight() + 14)
  toolTipLabel:show()
  toolTipLabel:raise()
  toolTipLabel:enable()
