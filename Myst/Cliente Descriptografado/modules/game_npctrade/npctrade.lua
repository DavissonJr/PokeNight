...
function onQuantityValueChange(quantity)
    if selectedItem then
        weightLabel:setText(string.format('%.2f', selectedItem.weight * quantity))
...
function setShowWeight(state)
    showWeight = state
    weightDesc:setVisible(false)
    weightLabel:setVisible(false)
end

function setShowYourCapacity(state)
    capacityDesc:setVisible(false)
...
function getCurrentTradeType()
    if tradeButton:getText() == tr('Comprar') then
        return BUY
    else
        return SELL
    end
...
function formatCurrency(amount)
    if CURRENCY_DECIMAL then
        return string.format('%.02f', amount * 0.01 / 100.0) .. ' ' .. CURRENCY
    else
        return amount * 0.01 .. ' ' .. CURRENCY
    end
end
