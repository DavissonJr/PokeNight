...
function init()
    ...
    --[[ consoleTextEdit.onTextChange = function(self, text)
        if g_game.isOnline() then
            local player = g_game.getLocalPlayer()
            if text == "" then
                player:setChatMessaging(0)
            end

            if text ~= "" then
                player:setChatMessaging(1)
            end
        end
    end ]]
...
function setEmojiText(emoji)
    consoleTextEdit:setText(consoleTextEdit:getText() .. " " .. emoji)
end
...
function addTab(name, focus)
    ...
    -- Thalles Vitor
    if not channelLists[name] then
        local chatTab = g_ui.createWidget("UIButton", modules.game_chatlist.getChatListPanel())
        chatTab.onClick = function()
            consoleTabBar:selectTab(tab)
            modules.game_chatlist.getChatList():hide()
        end

        local chatStatus = g_ui.createWidget("UIButton", chatTab)
        chatStatus:addAnchor(AnchorTop, "parent", AnchorTop)
        chatStatus:addAnchor(AnchorLeft, "parent", AnchorLeft)
        chatStatus:setSize("32 32")
        chatStatus:setImageSource("circle")

        local chatName = g_ui.createWidget("Label", chatTab)
        chatName:addAnchor(AnchorTop, "parent", AnchorTop)
        chatName:addAnchor(AnchorLeft, "parent", AnchorLeft)
        chatName:setText(name)
        chatName:setMarginLeft(55)
        chatName:setMarginTop(8)

        channelLists[name] = chatTab
    end
    return tab
end
...
function removeTab(tab)
    ...
    -- Thalles Vitor
    if channelLists[tab:getText()] then
        channelLists[tab:getText()]:destroy()
        channelLists[tab:getText()] = nil
    end
...
function sendMessage(message, tab)
    ...
    -- when talking on Registro do servidor, the message goes to default channel
...
function navigateMessageHistory(step)
    ...
    local player = g_game.getLocalPlayer()
    if player then
        player:lockWalk(0) -- lock walk for 200 ms to avoid walk during release of shift
    end
end
...
function online()
    ...
    serverTab = addTab(tr('Registro do servidor'), false)
...
function offline()
    ...
    for k, v in pairs(channelLists) do
        channelLists[k] = nil
    end
