...
function init() -- Initiating the module (load)
    ...
    battleButton = modules.client_topmenu.addRightGameToggleButton('battleButton', tr('Batalha') .. ' (Ctrl+B)',
        '/images/topbuttons/battle_hover', toggle)
    ...
    for i, v in pairs(hideButtons) do
        if v:isChecked() then
            if (i == 'hidePlayers' and creature:isPlayer()) or (i == 'hideNPCs' and creature:isNpc()) or
                (i == 'hideMonsters' and creature:isMonster() and creature:getMaster() == "") or
                (i == 'hideSkulls' and (creature:isMonster() and creature:getMaster() ~= "")) or
...
function enableAutoTarget()
    ...
    g_keyboard.bindKeyPress("Space", function()
        if not g_game.isOnline() then
            return 
        end

        g_game.getProtocolGame():sendExtendedOpcode(214)
    end, modules.game_interface.getRootPanel())
end

function disableAutoTarget()
    g_keyboard.unbindKeyPress("Space", modules.game_interface.getRootPanel())
end

ProtocolGame.registerExtendedOpcode(3, function(protocol, opcode, buffer)
    local param = buffer:explode("@")
    local creatureId = tonumber(param[1])

    --[[ if battleButtons[creatureId] then
        g_game.attack(battleButtons[creatureId].creature)
    end ]]
