...
-- Thalles Vitor
itemWidget = nil
function onItemMoving(item)
    if not item then
        return false
    end

    if not modules.client_options.getOption('showItemMove') then
        return true
    end

    if not g_mouse.isPressed(MouseLeftButton) then
        if itemWidget and itemWidget ~= nil then -- Thalles
            itemWidget:destroy()
            itemWidget = nil
        end
        return true
    ...
    for i = 1, 13 do
        if g_game.isOnline() then
            ...
            if i ~= 8 then
                local item2 = player:getInventoryItem(i)
                if item2 and item2:getId() == item:getId() then
                    return true
                end
            end
        end
    ...
    if not itemWidget then
        itemWidget = g_ui.createWidget("UIItem", modules.game_interface.getRootPanel())
    end

    itemWidget:setId("itemMoving")
    itemWidget:setSize("42 42")
...
function UIGameMap:onDrop(widget, mousePos)
    ...
    if modules.client_options.getOption('movePokemon') then
        if isInArray({3271, 3272, 3273, 3274, 3275, 3276, 3277}, thing:getId()) then
            dropWindow = displayGeneralBox(tr('Tem certeza?'), tr("Você quer mesmo derrubar essa pokébola?"), {
                {
                    text = tr('Yes'),
                    callback = function()
