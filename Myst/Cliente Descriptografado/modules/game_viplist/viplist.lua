...
function init()
    ...
    vipButton = modules.client_topmenu.addRightGameToggleButton('vipListButton', tr('Lista de Amigos') .. ' (Ctrl+P)',
                                                                '/images/topbuttons/viplist_hover', toggle)
...
function onAddVip(id, name, state, description, iconId, notify)
    ...
    local button = g_ui.createWidget('VIPButton', label)
    button.onMousePress = onVipListLabelMousePress
    button:addAnchor(AnchorTop, "parent", AnchorTop)
    button:addAnchor(AnchorLeft, "parent", AnchorLeft)
    button:setSize("170 15")
    button:setImageSource("list.png")
    button:setIcon("/images/game/status/online.png")
    button:setIconOffset("72 0")
    button:setMarginTop(12)
    button:setMarginLeft(-5)
    button:setText(name)
    button:setId('vips' .. id)
    ...
    if not g_game.getFeature(GameAdditionalVipInfo) then
        ...
        button.notifyLogin = notify
    end

    if state == VipState.Online then
        button:setColor('#00ff00')
        button:setIcon("/images/game/status/online.png")
    elseif button == VipState.Pending then
        button:setColor('#ffca38')
        button:setIcon("/images/game/status/ausente.png")
    else
        button:setColor('#ff0000')
        button:setIcon("/images/game/status/offline.png")
    end

    button.vipState = state
...
function onVipStateChange(id, state)
    local vipList = vipWindow:getChildById('contentsPanel')
    local label2 = vipList:getChildById('vip' .. id)
    local label = vipList:getChildById('vip' ..id):getChildById('vips' .. id)
...
function changeStatus(status)
    ...
    if player then
        g_game.getProtocolGame():sendExtendedOpcode(121, status.."@")
       -- player:setStatus("data/images/game/status/" .. status .. ".png")
    end
end