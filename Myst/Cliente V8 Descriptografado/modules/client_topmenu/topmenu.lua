...
local function addButton(id, description, icon, callback, panel, toggle, front)
    ...
    button.onHoverChange = function(self, value)
        onWidgetHoverChange(button, value)
        if value then
            if button:getId() == "audioButton" then
                if button:isOn() then
                    button:setIcon("/images/topbuttons/audio_hover")
                else
                    button:setIcon("/images/topbuttons/audio_mute_hover")
                end

                return true
            end

            if button:getId() == "talktypeButton" then
                if button:isOn() then
                    button:setIcon("/images/topbuttons/talktype/orange_marked_hover")
                else
                    button:setIcon("/images/topbuttons/talktype/orange")
                end

                return true
            end

         --[[    button:setIcon(icon .. "_hover.png") ]]
        else
            if button:getId() == "audioButton" then
                if button:isOn() then
                    button:setIcon("/images/topbuttons/audio")
                else
                    button:setIcon("/images/topbuttons/audio_mute")
                end
            ...
            if button:getId() == "talktypeButton" then
                if button:isOn() then
                    button:setIcon("/images/topbuttons/talktype/orange_marked")
                else
                    button:setIcon("/images/topbuttons/talktype/orange")
                end
...
function changeButtonIcon(id, panel, icon)
    local button = panel:getChildById(id)
    if button then
        button:setIcon(icon)
    end
