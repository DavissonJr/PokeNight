...
ProtocolGame.registerExtendedOpcode(46, function(protocol, opcode, buffer)
  ...
  if param[1] == "Teleport" then
    ...
    pokemonInfoWindowPokemonOrder.onClick = fixHabilitieFunction
  else
    if param[1] == nil then
      return false
    end
...
function exibir()
  if pokeInfoButton:isOn() == false then
    ...
    local function orderSystem()
      displayErrorBox(tr, 'Voc� n�o possui um pok�mon fora da pokebola, para ordenar.')
    end

    pokemonInfoWindow:show()
    pokeInfoButton:setOn(true)
    pokemonInfoWindowPokemonOrder.onClick = orderSystem
  else
    pokemonInfoWindow:hide()
    pokeInfoButton:setOn(false)
  end
