...
local config =
{
    ...
    outfitMale = 520, -- Outfit MALE
    outfitFemale = 521, -- Outfit FEMALE
...
function onUse(cid, item, itemEx, fromPosition, toPosition)
	...
	if item.uid ~= getPlayerSlotItem(cid, CONST_SLOT_RING).uid then
		doPlayerSendCancel(cid, "Você deve colocar sua roupa pascoal no local correto.") -- Mensagem que da ao tentar usar a bike fora do slot (by: Lukas)
	return TRUE
end

	if getPlayerStorageValue(cid, 17001) > 0 or getPlayerStorageValue(cid, 17000) > 0 or getPlayerStorageValue(cid, 63215) > 0 then
		doPlayerSendCancel(cid, "Você não pode usar o traje pascoal em situações especiais.")
	return true
end
