local config =
{
    velocidadeDaSuaBike = 3100, -- A volocidade da bike (1-9)
    outfitMale = 2947, -- Outfit male
    outfitFemale = 2948, -- Outfit female
}
...
return TRUE
    ...
    if isPlayer(cid) and getCreatureOutfit(cid).lookType == 814 then return false end
        if getPlayerStorageValue(cid, config.storageValue) <= 0 then
            ...
            setPlayerStorageValue(cid, 32724, ""..getPlayerStamina(cid).."")
            ...
            doSendMagicEffect(getThingPos(cid), 19)
