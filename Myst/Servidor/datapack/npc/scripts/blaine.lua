...
local function doWinDuel(cid, npc)
	...
	doCreatureSay(npc, "Você venceu o duelo! Parabéns, aceite este "..getItemNameById(a - 8).." como prêmio.", 1)
...
function onCreatureSay(cid, type, msg)
	...
	if msgcontains(msg, 'hi') and focus == 0 and getDistanceToCreature(cid) <= 4 then
		...
		talk_start = os.clock()
		conv = 1
		selfSay("Olá "..getCreatureName(cid)..", meu nome é Blaine e sou o líder do ginásio do Cinnabar. Como posso ajudá-lo?")
	return true
	end
	...
	if isDuelMsg(msg) and conv == 1 and focus == cid then
		...
		--	selfSay("Você já ganhou minha Insígnia do Vulcão, talvez algum outro dia possamos lutar.")
		--	focus = 0
		--return true
		--end

		if not hasPokemon(cid) then
			selfSay("Para lutar contra um líder de ginásio você precisa de pokémons.")
		return true
		end

		selfSay("Você está me desafiando para uma batalha. Será uma batalha "..#pokemons.." pokemon limit, vamos começar?")
		conv = 2
...
function onThink()
	...
	if fighting then
		...
		if not afk_warning and afk_time > afk_limit_time / 2 then
			selfSay("Onde está seu pokémon? Vamos lutar!  ")
			afk_warning = true
		end
		...
		if not hasPokemon(challenger) or challenger_turn >= 7 or challenger_turn > #pokemons then
			selfSay("Você perdeu nosso duelo! Talvez em outra hora você me derrote.")
			setPlayerStorageValue(focus, 990, -1)
			focus = 0
		return true
		end
